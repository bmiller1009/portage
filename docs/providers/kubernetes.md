...
No `mainApplicationFile` field exists on this CRD. The application entry point is expressed as `pyFiles` (a `local://` path baked into the container image) plus `driverArgs`; all Spark/Kubernetes configuration goes through the flat `sparkConf` map, including our `spark.portable.dataset.*.uri` keys. See `providers/execution/kubernetes/provider.py`'s `build_spark_application()` and `providers/execution/kubernetes/image/launcher.py` (the generic entry-point launcher baked into the base image).
