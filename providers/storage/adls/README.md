...
**Translation-layer prototype only** — no self-hostable open-source ADLS substitute and no real Azure subscription is available to this project, so this is tested against a fake `AdlsClientLike` client (`tests/unit/test_adls_provider.py`), never a real Azure account. The Databricks execution provider was in this same position until v0.3, when real workspace credentials became available and it moved to live-verified — ADLS remains blocked on Azure subscription access, unlike Databricks.
