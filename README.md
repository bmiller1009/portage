...
**Now in v0.1 ("Portable Spark Core").** The persistence layer is real (`control_plane/db.py`/`models.py`/`repositories.py`, migrated via `alembic/`), and so is the async submission path the spec describes (§24): `POST /v1/runs` persists `ACCEPTED` and returns immediately; `reconciler/service.py` picks it up, resolves the environment from the database, and submits to the real execution provider. This has been run live end-to-end — `POST /v1/runs` → reconciler → real SparkApplication on the remote Kubernetes cluster → `SUCCEEDED`, with verified-correct output in S3 — not just unit-tested. `plane run` (the CLI) still uses its original direct-provider path rather than this one; migrating it, plus cancel/logs endpoints and JVM JAR support, are the next backlog items — see the [v0.1 milestone](../../milestones).
